...
class Coupling_(Module_, ABC):
    ...
    def __init__(self, nets, *, mask, channels_axis=1):
        super().__init__()
    ...
    def reverse(self, x, log0=0):
        x = list(self.mask.split(x))  # x = [x_0, x_1]
        for k, net in enumerate(reversed(self.nets)):
            parity = (len(self.nets) - 1 - k) % 2
...
class RQSplineCoupling_(Coupling_):
    ...
    def __init__(
        self, nets, *, mask,
        xlim=(0, 1), ylim=(0, 1), knots_x=None, knots_y=None, extrap=None,
        channels_axis=1
    ):
    ...
    def atomic_forward(self, *, x_active, x_frozen, parity, net, log0=0):
        feature_map = net(self.preprocess_fz(x_frozen))
...
class MultiRQSplineCoupling_(Coupling_):
    ...
    # TODO: should be rewritten using make_rq_spline_field()
